...
function Profession(props, refs) {
  const { professionType = '11', professionName='擅长领域', form } = props;
  ...
  const getModalTitle = () => {
    if (isLevelOne) {
      if (currentItem) {
        return `修改${professionName}【一级分类】`;
      } else {
        return `新增${professionName}【一级分类】`;
      }
    } else {
      if (currentItem) {
        return `修改${professionName}【二级分类】`;
      } else {
        return `新增${professionName}【二级分类】`;
      }
    }
  };
  const okHandler = () => {
