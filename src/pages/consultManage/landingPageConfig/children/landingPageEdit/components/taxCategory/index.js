...
export default function TaxCategory(props) {
  const {
    serviceTypeList,
    serviceTypeMap,
    onPrevStep,
  ...
  // 获取专家咨询页面配置
  const getTaxPageConfig = async (businessId) => {
    const res = await get({
      url: Api.getOfficialServicePageConfig,
      params: {
        pageType: officialType.taxPage, // 页面类型（1：基础财税分类页；2：办税咨询分类页）
  ...
  useEffect(() => {
    if (lastData) {
      ...
      if (businessId && type === modelType.taxCategory) {
        getTaxPageConfig(businessId);
      } else {
        let data = {};
        let list = [];
        for (let i = 0; i < defaultBannerLength; i++) {
  ...
  return (
    <div className="tax-category-box">
      <OfficialServiceCategory
        ...
        officialTypeCode={officialType.tax}
        previewTypeObj={previewTypeMap.tax}
