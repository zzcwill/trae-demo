...
function EditModal(props, ref) {
      ...
      if (!err) {
        ...
        if (config.type === "edit") {
          updateRobotInteractConfig({
            id: config.data.id,
            satisfactionModelThreshold: sendData.satisfactionModelThreshold,
          });
        } else {
          sendData.brandList =
          ...
          sendData.locationList =
            sendData.locationList[0] === "all"
              ? undefined
              : sendData.locationList;
          addRobotInteractConfig(sendData);
        }
      }
    });
  };
   ...
   * 新增
   */
  const addRobotInteractConfig = async (data) => {
    try {
      const res = await post({
        url: Api.postAddRobotInteractConfig,
        data,
      });
      if (res.success) {
   ...
   * 修改
   */
  const updateRobotInteractConfig = async (data) => {
    try {
      const res = await post({
        url: Api.postUpdateRobotInteractConfig,
        data,
      });
      if (res.success) {
  ...
  return (
    <div className="robot-interact-modal-box">
      <Form>
        <div className="robot-interact-modal-label">条件设置</div>
        <Row>
          <Col span={11}>
        ...
        <div className="robot-interact-modal-label">参数设置</div>
        <Row>
          <FormItem label="满意度模型阈值" {...oneFormItemLayout}>
            {getFieldDecorator("satisfactionModelThreshold", {
              rules: [{ required: true, message: "请输入满意度模型阈值" }],
              initialValue:stringToNum(formData.satisfactionModelThreshold)
            })(
              <InputNumber
                min={0}
                max={1}
