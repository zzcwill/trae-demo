...
export default function RobotAssistantManage(props) {
   ...
   */
  const getManualInteractConfigList = (params) => {
    return get({
      url: Api.getManualInteractConfigList,
      params,
    });
  };
  ...
  const { params, getList, loading, total, changeParams, list } = useGetList({
    queryFunc: getManualInteractConfigList,
    defaultParam,
      ...
      render: (text, record, index) => {
        return (
          <div className="table-option-box">
            <span
              onClick={() => {
                editManualInteractConfig(record);
              }}
            ...
            <span
              onClick={() => {
                deleteManualInteractConfigButton(record.id);
              }}
              className="option-button"
            >
              删除
   ...
   * 删除按钮
   */
  const deleteManualInteractConfigButton = (id) => {
    Modal.confirm({
      title: "是否确定要删除",
      okText: "确认",
      cancelText: "取消",
   ...
   * 删除设置
   */
  const deleteManualInteractConfig = async (id) => {
    const res = await post({
      url: Api.postDeleteManualInteractConfig,
      data: {
        id,
      },
    });
   ...
   * 编辑
   */
  const editManualInteractConfig = (data) => {
    if (data) {
      setEditModal({
        isShow: true,
        name: "修改",
        ...
        data: {
          id: data.id,
          autoAnswerThreshold: data.autoAnswerThreshold,
  ...
  return (
    <div className="robot-assistant-manage-box">
      ...
      <div className="table-box">
        <div className="table-btn">
          <Button type="primary" onClick={() => editManualInteractConfig()}>
            新增
          </Button>
        </div>
        <Table
