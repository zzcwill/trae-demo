...
import {Radio, Input, Button, message} from 'dpl-react'
    ...
    return <div className='advert'>
        ...
        <div className='advert-upload-image-wrap'>
            <UploadImage
                ...
                onChange={(data) => {
                    if (data.length === 0) {
                        onChange({adType: value.adType, adUrl: ''})
                    } else {
                        onChange({adType: value.adType, adUrl: data[0].imageUrl})
                    }
                }}
            />
        ...
        <div className='advert-url-wrap'>
            <Input value={value.adUrl}
                   disabled={disabled}
                   onChange={(e) => {
                       onChange({adType: value.adType, adUrl: e.target.value})
                   }}
                   maxLength={200}
            />
            <span className='tips'>请使用https的链接</span>
        </div>
        }
    </div>
}
